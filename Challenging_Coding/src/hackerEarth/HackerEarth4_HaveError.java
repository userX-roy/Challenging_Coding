...
public class HackerEarth4_HaveError {
	static Scanner sc = new Scanner(System.in);

	int sol(int zeroCount, int oneCount, int setOfOne) {
	...
	void meth1(int testCase) {
		...
		while (i < testCase) {
			...
			resultArr[i] = sol(zeroCount, oneCount, setOfOne);
			i++;
		}
		for (int j : resultArr) {
	...
	public static void main(String[] args) {
		new HackerEarth4_HaveError().meth1(sc.nextInt());
	}
}
